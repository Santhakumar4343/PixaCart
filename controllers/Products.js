...
PRODUCTS.create_product = async (req, res) => {
    ...
    if(req.body.id && req.body.id != 0) {
        ...
        await productsModel.findOneAndUpdate({'_id':req.body.id},postData).then(async result=>{
            ...
            res.status(200).json({ status: 1 , message: 'Product Updated Successfully!', 'data': req.body.id ,'redirect' : 'dashboard/products' });
        }).catch(err=>{
        ...
        await  productsModel.create(postData).then(creatRes=>{ 
            res.status(200).json({ status: 1 , message: 'Product Added Successfully!', 'data': creatRes ,'redirect' : 'dashboard/add-product-variant/'+creatRes._id });
        }).catch(err=>{
...
PRODUCTS.create_product_variants = async (req, res) => {
    ...
    if(req.body.prod_vid && req.body.prod_vid != 0) {
        ...
        postData.updatedAt = Date.now();
        await productsVariantsModel.findOneAndUpdate({_id:req.body.prod_vid , prod_id :  postData.prod_id},postData).then(async result=>{
            if(req.files){
                let imageInsert = [];
                for (let i = 0; i < req.files.length; i++) {
                   let imagesData = {      
...
PRODUCTS.deleteProductVariantThumb = async (req, res) => { 
    ...
    let ThumbDetails = await productsThumbModel.countDocuments({prod_variant_id : varnt_id});
    if(ThumbDetails > 1){
        productsThumbModel.findOneAndDelete({'_id':thumb_id}, (err, previousImage) => {
            if(err) {
                ...
                res.send({ status: 1,'message':'Thumb Deleted Successfully','data':''})
            }
        });
    }else{
        res.send({ status: 0,'message':'Unable to delete this last thumb.','data':''})
    }
...
PRODUCTS.create_category = async (req, res)  => {
    ...
    if(req.body.id && req.body.id != 0) {
        await categoryModel.findOne({'cate_name':req.body.cate_name,'_id': {$ne: req.body.id}}).then(async checkCateExist=>{
            if (checkCateExist) {
                res.status(401).json({status:0 , message:'Category already exists.',data:''})
            } else {      
        ...
        await categoryModel.findOne({'cate_name':req.body.cate_name}).then(check=>{
            if (check) {
                res.status(401).json({status: 0 ,message:'Category already exists.',data:check})
            } else {
...
PRODUCTS.create_attribute = async (req, res)  => {
     ...
     if(req.body.id && req.body.id != 0) {
         await attributesModel.findOne({'attribute_name':req.body.attribute_name,'_id': {$ne: req.body.id}, 'attribute_cate': req.body.attributes_cate }).then(async checkExist=>{
            if (checkExist) {
                res.status(401).json({status:0 , message:'Attribute name already exists, Please try another one.',data:''})
            } else {                    
            postData.updatedAt = Date.now();
            await attributesModel.findOneAndUpdate({'_id':req.body.id},postData).then(async result=>{
                res.status(200).json({ status: 1 , message: 'Attribute Updated Successfully!', 'data': req.body.id });
            }).catch(err=>{
                res.status(500).json({ status:0 , message: 'Something went wrong in update attribute.', data: err.message });                    
            });
            }
         }).catch(err=>{
             res.status(500).json({ status: 0, message: 'Something went wrong in update attribute.', data: err.message });
         ...
         await attributesModel.findOne({'attribute_name':req.body.attribute_name, 'attribute_cate': req.body.attributes_cate}).then(check=>{
             if (check) {
                 res.status(401).json({status: 0 ,message:'Attribute name already exists, Please try another one.',data:check})
             } else {
                attributesModel.create(postData).then(creatRes=>{
                     res.status(200).json({ status: 1 , message: 'Attribute Added Successfully!', 'data': creatRes });
                 }).catch(err=>{
                     res.status(500).json({ status: 0 , message: 'Something went wrong in insert Attribute.', data: err.message });
                 })
             }
         })
     }
...
PRODUCTS.create_banner = async (req, res)  => {
    ...
    if(req.file){
        ...
        await helper.createThumb(req.file, width = 80, height = 50, uploadPath = filePath);
    }
 
    postData.user_id =  req.session.user.user_id;
    postData.banner_link = req.body.banner_link;
    promotionalBannerModel.create(postData).then(creatRes=>{
        res.status(200).json({ status: 1 , message: 'Banner Added Successfully!', 'data': creatRes });
    }).catch(err=>{
        res.status(500).json({ status: 0 , message: 'Something went wrong in insert banner.', data: err.message });
    });
 }
...
PRODUCTS.subCategorList = async (req, res) => {
  ...
  PRODUCTS.attributesList = async (req, res) => {
    try {
        ...
        await attributesModel.find(query).populate('attribute_cate').skip(start).limit(dataLimit).sort({ _id : 'desc' }).then(async (result)=>{
            var mytable = {
                draw:req.query.draw,
                recordsTotal:0,
                recordsFiltered:0,
           ...
           if(result.length > 0){
                result.forEach(function(element,key) {
                    ...
                    ulist += '</ol>';
                   let cateName =  (element.attribute_cate) ? element.attribute_cate.cate_name : '';
                   let checked =  (element.status) ? 'checked' : '';
                    mytable.data[key] = [ ++start,
  ...
  PRODUCTS.productsList = async (req, res) => {
    try {
        ...
        let uid         = typeof req.session.user != 'undefined' ? mongoose.Types.ObjectId(req.session.user.user_id)  : mongoose.Types.ObjectId()
        let mquery      = [];
  ...
  PRODUCTS.productsVariantsList = async (req, res) => {
    try {
        if(req.query.prod_id && req.query.prod_id != 0) {
            let query       = { 'prod_sellerid' : await helper.uid(req) , 'prod_id' :  req.query.prod_id };
                ...
                if(result.length > 0){
                    ...
                    result.forEach( function(element,key) {
                        ...
                        MyTbl +=`<tr>
                                    ...
                                    <td><a href="/dashboard/add-product-variant/${element.prod_id}/${element._id}" title="Edit Variant"><i class="fas fa-edit"></i></a></td>
                                </tr>` 
                    }); 
            ...
            res.status(401).json({ status : 0, message : 'error '+ 'Product id is required' });
        }
      }catch (err) {
        res.status(401).json({ status : 0, message : 'error '+ err });
      }
  ...
  PRODUCTS.reviewsList = async (req, res) => {
    try {
        ...
        await ratingReviewModel.find(query).populate({path: 'rating_pid', match : query2 } ).populate('rating_uid').skip(start).limit(dataLimit).sort({ _id : 'desc' }).then(async (result)=>{
            var mytable = {
                draw:req.query.draw,
                recordsTotal:0,
                recordsFiltered:0,
  ...
  PRODUCTS.wishList_old = async (req, res) => {
    try {
        ...
        await likesModel.find(query).populate('like_pid').skip(start).limit(dataLimit).sort({ _id : 'desc' }).then(async (result)=>{
            var mytable = {
                draw:req.query.draw,
                recordsTotal:0,
                recordsFiltered:0,
  ...
  PRODUCTS.bannersList = async (req, res) => {
    try {
        ...
        await promotionalBannerModel.find(query).skip(start).limit(dataLimit).sort({ _id : 'desc' }).then(async (result)=>{
            var mytable = {
                draw:req.query.draw,
                recordsTotal:0,
                recordsFiltered:0,
            ...
            mytable.recordsFiltered = await promotionalBannerModel.countDocuments(query);
           ...
           if(result.length > 0){
                result.forEach(function(element,key) {
                    ...
                    if(element.banner_image != null){
                        banner_image = config.APP_URL+'uploads/banners/'+element.banner_image; 
                       let fileArr  = element.banner_image.split('.');
                       thumb_image  = config.APP_URL+'uploads/banners/'+fileArr[0]+'_thumb.'+fileArr[1];
                    }else{
  ...
  PRODUCTS.deleteBanner = async (req, res) => {
        ...
        filePath = './public/uploads/banners/';
        promotionalBannerModel.findOneAndDelete({'_id':banner_id}, (err, previousImage) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                //let  previousImage = await brandModel.findOne({_id: req.body.id}).select("brand_image").exec();
               ...
               // console.log(cust)
                res.send({ status: 1,'message':'Banner Deleted Successfully','data':''})
            }
        });
    };
    ...
    PRODUCTS.updateCateStatus = async (req, res) => {
        ...
        categoryModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
    ...
    PRODUCTS.updateSubcateStatus = async (req, res) => {
        ...
        subCategoryModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                res.send({ status: 1,'message':'Status updated Successfully.','data':result});
    ...
    PRODUCTS.updateAttributeStatus = async (req, res) => {
        ...
        attributesModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                res.send({ status: 1,'message':'Status updated Successfully.','data':result});
    ...
    PRODUCTS.updateBrandStatus = async (req, res) => {
        ...
        brandModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                res.send({ status: 1,'message':'Status updated Successfully.','data':result});
    ...
    PRODUCTS.updateBannerStatus = async (req, res) => {
        ...
        promotionalBannerModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                res.send({ status: 1,'message':'Status updated Successfully.','data':result});
    ...
    PRODUCTS.updateProductStatus = async (req, res) => {
        ...
        productsModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                res.send({ status: 1,'message':'Status updated Successfully.','data':result});
    ...
    PRODUCTS.updateProductVariantStatus = async (req, res) => {
        ...
        productsVariantsModel.findOneAndUpdate({'_id':req.body.id},{status : req.body.status}, (err, result) => {
            if(err) {
                res.send({ status:0,'message':err,'data':''})
            } else {
                res.send({ status: 1,'message':'Status updated Successfully.','data':result});
